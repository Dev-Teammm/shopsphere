...
public class StockBatch {
    ...
    /**
     * Absolute expiration timestamp - source of truth for batch expiration at
     * runtime
     * When expiresAt <= now, the batch is considered expired
     * This field is the authoritative source for determining batch availability
     * The background job periodically marks batches as EXPIRED using this timestamp
     */
    ...
    // ==================== Category-specific fields (Batch-level)
    // ====================
    // These fields are specific to each batch and can vary between batches.
    // Note: harvest date is represented by manufactureDate (when the batch was
    // produced/harvested).

    /**
     * Farm or region of origin for this specific batch
     * Different batches can come from different farms/suppliers
     */
    @Column(name = "origin_farm")
    private String originFarm;

    /**
     * Quality grade for this specific batch (e.g., "Grade A", "Premium",
     * "Standard")
     * Different batches can have different quality grades
     */
    @Column(name = "grade_quality")
    ...
    public void updateStatus() {
        ...
        // Check if batch has expired using expiresAt (source of truth)
        if (expiresAt != null && expiresAt.isBefore(LocalDateTime.now())) {
            status = BatchStatus.EXPIRED;
            return;
        }

        // Check if batch has expired using expiryDate (legacy field)
     ...
     * Uses expiresAt (source of truth) if available, otherwise falls back to
     * expiryDate
     * 
     * @return true if expiration timestamp has passed
     */
    ...
    public boolean isExpiringSoon(int daysThreshold) {
        ...
        // Check expiresAt first (source of truth)
        if (expiresAt != null) {
            return expiresAt.isBefore(thresholdDateTime) && !isExpired();
        }
