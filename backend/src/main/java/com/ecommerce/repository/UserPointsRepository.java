...
public interface UserPointsRepository extends JpaRepository<UserPoints, Long> {
    ...
    /**
     * Find current point balance for a user scoped to each shop
     * Returns a list of objects where [0] is Shop and [1] is Sum of Points
     */
    @Query("SELECT up.shop, SUM(up.points) FROM UserPoints up WHERE up.user.id = :userId GROUP BY up.shop")
    List<Object[]> findShopPointsSummaryByUserId(@Param("userId") UUID userId);
