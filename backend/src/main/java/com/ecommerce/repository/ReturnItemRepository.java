...
public interface ReturnItemRepository extends JpaRepository<ReturnItem, Long> {
    ...
    /**
     * Check if there are any pending return requests that include this product
     * (either as direct product or via variant). Used to block soft-delete.
     */
    @Query("SELECT COUNT(ri) > 0 FROM ReturnItem ri WHERE ri.returnRequest.status = 'PENDING' " +
           "AND (ri.product.productId = :productId OR (ri.productVariant IS NOT NULL AND ri.productVariant.product.productId = :productId))")
    boolean existsPendingReturnByProductId(@Param("productId") UUID productId);
