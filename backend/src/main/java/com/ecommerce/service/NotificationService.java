...
public class NotificationService {
    ...
    /**
     * Overloaded method for manual refund processing
     */
    public void notifyRefundProcessed(ReturnRequest returnRequest) {
        Double amount = returnRequest.getRefundAmount() != null
                ? returnRequest.getRefundAmount().doubleValue()
                : 0.0;
        String refundMethod = "Manual Transfer";

        log.info("Sending manual refund processed notification to customer {} for request {} - amount: {}",
                returnRequest.getCustomerId(), returnRequest.getId(), amount);

        // emailService.sendRefundProcessedEmail(returnRequest, refundMethod, amount);

        log.info("Manual refund processed notification sent for request {}", returnRequest.getId());
    }
    ...
    private String buildReturnRequestSubmittedEmailBody(String customerName,
        ...
        body.append("Shopsphere Marketplace Platform");
