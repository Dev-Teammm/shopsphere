...
public class AnalyticsServiceImpl implements AnalyticsService {
    ...
    private AnalyticsResponseDTO getShopAnalytics(UUID shopId, LocalDateTime from, LocalDateTime to,
        ...
        // New Shop Members (employees and delivery agents) in the period
        // Use Specification for efficient querying
        Specification<User> memberSpec = (root, query, cb) -> {
            List<jakarta.persistence.criteria.Predicate> predicates = new ArrayList<>();
            predicates.add(cb.equal(root.get("shop").get("shopId"), shopId));
            predicates.add(cb.or(
                    cb.equal(root.get("role"), UserRole.EMPLOYEE),
                    cb.equal(root.get("role"), UserRole.DELIVERY_AGENT)
            ));
            predicates.add(cb.between(root.get("createdAt"), from, to));
            return cb.and(predicates.toArray(new jakarta.persistence.criteria.Predicate[0]));
        };
        
        long newShopMembers = userRepository.count(memberSpec);
        
        // Previous period counts
        Specification<User> prevMemberSpec = (root, query, cb) -> {
            List<jakarta.persistence.criteria.Predicate> predicates = new ArrayList<>();
            predicates.add(cb.equal(root.get("shop").get("shopId"), shopId));
            predicates.add(cb.or(
                    cb.equal(root.get("role"), UserRole.EMPLOYEE),
            ...
            predicates.add(cb.between(root.get("createdAt"), prevFrom, prevTo));
            return cb.and(predicates.toArray(new jakarta.persistence.criteria.Predicate[0]));
        };
        
        long prevNewShopMembers = userRepository.count(prevMemberSpec);
        Double newShopMembersVs = percentChange(prevNewShopMembers, newShopMembers);
        ...
        return AnalyticsResponseDTO.builder()
                ...
                .newShopMembers(newShopMembers)
                .newShopMembersVsPercent(newShopMembersVs)
                .topProducts(top)
