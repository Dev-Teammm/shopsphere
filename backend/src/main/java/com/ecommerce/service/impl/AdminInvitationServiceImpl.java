...
public class AdminInvitationServiceImpl implements AdminInvitationService {
    ...
    public AdminInvitationDTO createInvitation(UUID vendorId, CreateAdminInvitationDTO createInvitationDTO) {
        ...
        Shop shop = shopRepository.findById(createInvitationDTO.getShopId())
                .orElseThrow(() -> new EntityNotFoundException(
                        "Shop not found with ID: " + createInvitationDTO.getShopId()));

        // Check for existing user status first
        User existingUserCheck = userRepository.findByUserEmail(createInvitationDTO.getEmail()).orElse(null);
        if (existingUserCheck != null) {
            boolean isCustomer = existingUserCheck.getRole() == UserRole.CUSTOMER;
            boolean belongsToThisShop = existingUserCheck.getShop() != null
                    && existingUserCheck.getShop().getShopId().equals(shop.getShopId());
            boolean belongsToAnotherShop = existingUserCheck.getShop() != null
                    && !existingUserCheck.getShop().getShopId().equals(shop.getShopId());

            if (belongsToThisShop && !isCustomer) {
                throw new IllegalArgumentException(
                        "User is already a member of this shop. Please go to the Members tab to manage their role.");
            }

            if ((belongsToAnotherShop || (!belongsToThisShop && existingUserCheck.getShop() == null)) && !isCustomer) {
                String existingShopName = existingUserCheck.getShop() != null ? existingUserCheck.getShop().getName()
                        : "another organization";
                throw new IllegalArgumentException("User belongs to " + existingShopName
                        + ". They must be released by that shop before you can invite them.");
            }
        }
        ...
        if (assignedRole == UserRole.ADMIN) {
            throw new IllegalArgumentException(
                    "ADMIN role cannot be invited. Only VENDOR, EMPLOYEE, and DELIVERY_AGENT roles can be invited to shops.");
        }

        // Only allow VENDOR, EMPLOYEE, and DELIVERY_AGENT
        if (assignedRole != UserRole.VENDOR && assignedRole != UserRole.EMPLOYEE
                && assignedRole != UserRole.DELIVERY_AGENT) {
            throw new IllegalArgumentException(
                    "Only VENDOR, EMPLOYEE, and DELIVERY_AGENT roles can be invited. Role provided: " + assignedRole);
        }
        ...
        try {
            if (existingUser != null) {
                // Send role update email to existing user with invitation link
                ...
                log.info("Role update email with invitation link sent to existing user: {}", createInvitationDTO.getEmail());
            } else {
    ...
    public AdminInvitationDTO updateInvitation(UUID invitationId, UpdateAdminInvitationDTO updateInvitationDTO) {
        ...
        if (updateInvitationDTO.getAssignedRole() != null) {
            try {
                ...
                if (assignedRole == UserRole.ADMIN) {
                    throw new IllegalArgumentException(
                            "ADMIN role cannot be assigned. Only VENDOR, EMPLOYEE, and DELIVERY_AGENT roles can be assigned to shops.");
                }
                ...
                if (assignedRole != UserRole.VENDOR && assignedRole != UserRole.EMPLOYEE
                        && assignedRole != UserRole.DELIVERY_AGENT) {
                    throw new IllegalArgumentException(
                            "Only VENDOR, EMPLOYEE, and DELIVERY_AGENT roles can be assigned. Role provided: "
                                    + assignedRole);
                }
                ...
                throw new IllegalArgumentException(
                        "Invalid role: " + updateInvitationDTO.getAssignedRole() + ". " + e.getMessage());
            }
        }
    ...
    public boolean deleteInvitation(UUID invitationId) {
        ...
        if (invitation.getStatus() == AdminInvitation.InvitationStatus.ACCEPTED) {
            throw new IllegalStateException("Cannot delete an invitation that has already been ACCEPTED.");
        }
    ...
    public AdminInvitationDTO acceptInvitation(AcceptInvitationDTO acceptInvitationDTO) {
        ...
        if (existingUser != null) {
            ...
            if (acceptInvitationDTO.getPassword() == null || acceptInvitationDTO.getPassword().trim().isEmpty()) {
                throw new IllegalArgumentException(
                        "Password is required when creating a new account. The user with email " + invitation.getEmail()
