...
public class DashboardServiceImpl implements DashboardService {
        ...
        private final com.ecommerce.repository.ReturnRequestRepository returnRequestRepository;
        private final com.ecommerce.repository.ReturnAppealRepository returnAppealRepository;
        ...
        public DashboardResponseDTO getDashboardData(String bearerToken, String shopSlug) {
                ...
                if (shopSlug != null && !shopSlug.trim().isEmpty()) {
                        shop = shopRepository.findBySlug(shopSlug)
                                        .orElseThrow(() -> new IllegalArgumentException(
                                                        "Shop not found with slug: " + shopSlug));
                        ...
                        if (role != UserRole.ADMIN) {
                                throw new IllegalArgumentException(
                                                "shopSlug is required for VENDOR and EMPLOYEE roles");
                        }
                }
                ...
                if (shopId != null) {
                        ...
                        // New: Pending returns and appeals for shop
                        pendingReturns = returnRequestRepository.countByShopOrderShopShopIdAndStatus(shopId,
                                        com.ecommerce.entity.ReturnRequest.ReturnStatus.PENDING);
                        pendingAppeals = returnAppealRepository.countByShopIdAndStatus(shopId,
                                        com.ecommerce.entity.ReturnAppeal.AppealStatus.PENDING);
                        ...
                        recentOrders = shopOrders.stream()
                                        .map(so -> RecentOrderDTO.builder()
                                                        ...
                                                        .owner(so.getOrder().getUser() != null
                                                                        ? so.getOrder().getUser().getFullName()
                                                                        : "Guest")
                                                        .build())
                                        .collect(Collectors.toList());

                        // Count shop members by role
                        long totalEmployees = userRepository.findByRoleAndShop(UserRole.EMPLOYEE, shopId, PageRequest.of(0, Integer.MAX_VALUE)).getTotalElements();
                        long totalDeliveryAgents = userRepository.findByRoleAndShop(UserRole.DELIVERY_AGENT, shopId, PageRequest.of(0, Integer.MAX_VALUE)).getTotalElements();
                        ...
                        if (role == UserRole.ADMIN || shopAuthorizationService.canManageShop(userId, shopId)) {
                                ...
                                DashboardResponseDTO.DashboardResponseDTOBuilder builder = DashboardResponseDTO
                                                .builder()
                                                .totalProducts(totalProducts)
                                                .totalOrders(totalOrders)
                                                .totalRevenue(revenue)
                                                .pointsRevenue(shopOrderRepository
                                                                .sumTotalPointsRevenueByShopId(shopId))
                        ...
                        pendingOrders = shopOrderRepository
                                        .countByStatus(com.ecommerce.entity.ShopOrder.ShopOrderStatus.PENDING);
                        lowStock = productRepository.countLowStock();

                        // New: Global pending returns and appeals
                        pendingReturns = returnRequestRepository
                                        .countByStatus(com.ecommerce.entity.ReturnRequest.ReturnStatus.PENDING);
                        pendingAppeals = returnAppealRepository
                                        .countByStatus(com.ecommerce.entity.ReturnAppeal.AppealStatus.PENDING);
                        ...
                        recentOrders = globalOrders.stream()
                                        .map(o -> RecentOrderDTO.builder()
                                                        ...
                                                        .status(o.getStatus())
                                                        .amount(o.getOrderTransaction() != null
                                                                        ? o.getOrderTransaction().getOrderAmount()
                                                                        : (o.getOrderInfo() != null ? o.getOrderInfo()
                                                                                        .getTotalAmount()
                                                                                        : BigDecimal.ZERO))
                                                        .owner(o.getUser() != null ? o.getUser().getFullName()
                                                                        : "Guest")
                                                        .build())
                                        .collect(Collectors.toList());
