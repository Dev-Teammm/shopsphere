...
public class WarehouseServiceImpl implements WarehouseService {
    ...
    @Override
    public Page<WarehouseDTO> searchWarehouses(String query, Pageable pageable) {
        return searchWarehouses(query, null, pageable);
    }

    @Override
    public Page<WarehouseDTO> searchWarehouses(String query, UUID shopId, Pageable pageable) {
        log.info("Searching warehouses with query: {} and shopId: {}", query, shopId);
        return warehouseRepository.searchWarehouses(query, shopId, pageable)
    ...
    public WarehouseDTO createWarehouse(CreateWarehouseDTO createWarehouseDTO, List<MultipartFile> warehouseImages) {
        try {
            ...
            if (createWarehouseDTO.getShopId() != null) {
                // Check for name uniqueness within the same shop
                if (warehouseRepository.existsByNameAndShopShopId(createWarehouseDTO.getName(),
                        createWarehouseDTO.getShopId())) {
                    throw new IllegalArgumentException("A warehouse with the name '" + createWarehouseDTO.getName()
                            + "' already exists in this shop.");
                }

                Shop shop = shopRepository.findById(createWarehouseDTO.getShopId())
                        .orElseThrow(() -> new EntityNotFoundException(
                                "Shop not found with ID: " + createWarehouseDTO.getShopId()));
    ...
    public WarehouseDTO updateWarehouse(Long warehouseId, UpdateWarehouseDTO updateWarehouseDTO,
        ...
        try {
            ...
            if (updateWarehouseDTO.getName() != null && !updateWarehouseDTO.getName().equals(warehouse.getName())) {
                // Check if the new name already exists in this shop
                if (warehouseRepository.existsByNameAndShopShopId(updateWarehouseDTO.getName(),
                        warehouse.getShop().getShopId())) {
                    throw new IllegalArgumentException("A warehouse with the name '" + updateWarehouseDTO.getName()
                            + "' already exists in this shop.");
                }
        ...
        try {
            ...
            if (mainProductStockOpt.isPresent()) {
                ...
                if (!batches.isEmpty()) {
                    ...
                    // First remove OrderItemBatch references to avoid foreign key constraint
                    // violations
                    for (StockBatch batch : batches) {
                        List<OrderItemBatch> orderItemBatches = orderItemBatchRepository
                                .findByStockBatch_Id(batch.getId());
    ...
    private WarehouseProductDTO buildWarehouseProductDTO(Product mainProduct, List<Stock> productStocks) {
        log.debug("Building DTO for product: {} with {} stock entries", mainProduct.getProductName(),
                productStocks.size());
        ...
        return WarehouseProductDTO.builder()
                ...
                .productImages(mainProduct.getImages() != null
                        ? mainProduct.getImages().stream()
                                ...
                                .collect(Collectors.toList())
                        : List.of())
                .unit(com.ecommerce.dto.UnitDTO.from(mainProduct.getUnit()))
                .organic(mainProduct.getOrganic())
                .build();
    }
}
