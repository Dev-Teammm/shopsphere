...
public class ShippingCostServiceImpl implements ShippingCostService {
    ...
    public BigDecimal calculateOrderShippingCost(AddressDto deliveryAddress, List<CartItemDTO> items,
        ...
        com.ecommerce.dto.ShippingDetailsDTO details = calculateEnhancedShippingDetails(deliveryAddress, items,
                orderValue);
    ...
    public ShippingCostDTO toggleShippingCostStatus(Long id, UUID shopId) {
        ...
        if (Boolean.TRUE.equals(shippingCost.getIsActive())) {
            ...
            shippingCostRepository.deactivateAllShippingCostsByShopId(shopId);
            shippingCost.setIsActive(true);
            log.info("Activated shipping cost: {} and deactivated all others for shop {}", shippingCost.getName(),
                    shopId);
        }
    ...
    /**
     * Get customer location coordinates with fallback handling
     */
    private CustomerLocation getCustomerLocation(AddressDto deliveryAddress) {
        // Always prefer provided coordinates
        ...
        log.info("No coordinates provided, attempting geocoding service");
        try {
            ...
            if (coords != null) {
                ...
                log.info("Successfully geocoded address to coordinates: ({}, {})", lat, lon);
                return new CustomerLocation(lat, lon);
            }
        } catch (Exception e) {
            log.warn("Geocoding failed: {}. This may be due to service rate limiting or network issues.",
                    e.getMessage());
        }

        // Fallback: Use country center if available (Rwanda center coordinates as
        // example)
        String country = deliveryAddress.getCountry();
        if (country != null && country.equalsIgnoreCase("Rwanda")) {
            log.info("Using fallback: Rwanda country center coordinates");
            // Rwanda approximate center: -2.0000, 30.0000
            return new CustomerLocation(-2.0, 30.0);
        }

        // Cannot determine location
        log.error("Could not determine customer location for address: {}, Country: {}",
                deliveryAddress.getStreetAddress(), country);
        throw new IllegalStateException(
                "Unable to calculate shipping: customer location could not be determined. " +
                        "Please verify your address details or try again later.");
    }
    ...
    private Product getEffectiveProduct(CartItemDTO item) {
        if (item.isVariantBasedItem() && item.getVariantId() != null) {
            ProductVariant variant = productVariantRepository.findById(item.getVariantId())
                    .orElseThrow(
                            () -> new EntityNotFoundException("Variant not found with ID: " + item.getVariantId()));
            ...
            return productRepository.findById(item.getProductId())
                    .orElseThrow(
                            () -> new EntityNotFoundException("Product not found with ID: " + item.getProductId()));
        }
