...
The services handle the management of product attributes, which are used to define variants of products. For example, a T-shirt product might have attributes like "Color" and "Size", with values like "Red", "green", "M", "L", etc.
...
Manages attribute values (e.g., "Red", "green", "M", "L").
...
3. Check if attribute values ("Red", "green", "M", "L") exist using `attributeValueExists` or `getAttributeValueByValueAndTypeId`
4. If not, add them using `createAttributeValue`
5. Use these values when creating ProductVariant entries (e.g., "Red, M", "Red, L", "green, M")
