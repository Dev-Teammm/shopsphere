 ...
 * This ensures database schema is updated to support all payment methods
 * including HYBRID.
 */
@Component
...
public class DatabaseMigrationConfig {
    ...
    /**
     * Add HYBRID payment method to the order_transactions payment_method check
     * constraint.
    ...
    private void addHybridPaymentMethodSupport() {
        try {
            ...
            String checkConstraintSql = "SELECT COUNT(*) FROM information_schema.constraint_column_usage " +
            ...
            if (constraintExists != null && constraintExists > 0) {
                ...
                String dropConstraintSql = "ALTER TABLE order_transactions DROP CONSTRAINT IF EXISTS order_transactions_payment_method_check";
                ...
                String addConstraintSql = "ALTER TABLE order_transactions " +
            ...
            String verifyConstraintSql = "SELECT pg_get_constraintdef(oid) " +
                    "FROM pg_constraint " +
    ...
    /**
     * Make warehouse address fields (state, zip_code, street_number) nullable.
     * This is required because these fields are no longer required in Rwanda.
     */
    private void fixWarehouseAddressFields() {
        try {
            log.info("Checking and fixing warehouse address fields constraints...");

            // PostgreSQL syntax to drop NOT NULL constraints
            String[] updateQueries = {
                    "ALTER TABLE warehouses ALTER COLUMN state DROP NOT NULL",
                    "ALTER TABLE warehouses ALTER COLUMN zip_code DROP NOT NULL",
                    "ALTER TABLE warehouses ALTER COLUMN street_number DROP NOT NULL"
            };
            ...
            for (String sql : updateQueries) {
                try {
                    ...
                    // Log but continue - the column might already be nullable or might not exist
                    log.debug("Note: Could not execute '{}': {}", sql, e.getMessage());
                }
            }
            log.info("Warehouse address fields migration completed");

        } catch (Exception e) {
            log.warn("Error during warehouse address fields migration: {}", e.getMessage());
        }
    }
}
