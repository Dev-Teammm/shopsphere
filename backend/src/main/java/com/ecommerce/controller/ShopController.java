...
public class ShopController {
    ...
    public ResponseEntity<?> getShopDetails(
        ...
        try {
            // Get userId from authentication if available (optional for public access)
            UUID userId = null;
            if (authentication != null && authentication.isAuthenticated()) {
                try {
                    userId = getCurrentUserId();
                } catch (Exception e) {
                    log.debug("Could not get userId from authentication: {}", e.getMessage());
                }
            }
            
            com.ecommerce.dto.ShopDetailsDTO details = shopService.getShopDetails(shopId, userId);
    ...
    public ResponseEntity<?> searchShops(
        ...
        try {
            ...
            if ("followers".equals(sortBy)) {
                // For followers, we'll sort in memory after fetching, so use unsorted for the query
                // This avoids the column name translation issue with native queries
                sorting = Sort.unsorted();
            } else {
                // For native queries, we need to use the actual database column names
                // Map entity property names to database column names
                String dbColumnName = switch (sortBy) {
                    case "rating" -> "rating";
                    case "products" -> "product_count";
                    case "name" -> "shop_name";
                    default -> "created_at";
                };
                sorting = sortDirection.equalsIgnoreCase("asc") ? Sort.by(dbColumnName).ascending()
                        : Sort.by(dbColumnName).descending();
            }

            Pageable pageable = PageRequest.of(page, size, sorting);
            
            // Get userId from authentication if available
            UUID userId = null;
            if (authentication != null && authentication.isAuthenticated()) {
                try {
                    userId = getCurrentUserId();
            ...
            // If followedOnly is true but no userId, return empty
            if (Boolean.TRUE.equals(followedOnly) && userId == null) {
                return ResponseEntity.ok(new org.springframework.data.domain.PageImpl<>(java.util.Collections.emptyList(), pageable, 0));
            }
            
            Page<ShopDTO> shops = shopService.searchShops(search, category, followedOnly, userId, pageable);
            
            // If sorting by followers, sort the results in memory
            if ("followers".equals(sortBy)) {
                List<ShopDTO> sortedShops = shops.getContent().stream()
                        .sorted((s1, s2) -> {
                            long count1 = s1.getFollowerCount() != null ? s1.getFollowerCount() : 0;
                            long count2 = s2.getFollowerCount() != null ? s2.getFollowerCount() : 0;
                            return sortDirection.equalsIgnoreCase("asc") 
                                    ? Long.compare(count1, count2) 
                                    : Long.compare(count2, count1);
                        })
                        .collect(java.util.stream.Collectors.toList());
                shops = new org.springframework.data.domain.PageImpl<>(sortedShops, pageable, shops.getTotalElements());
            }
    ...
    @Operation(summary = "Get countries with delivery availability", description = "Get all countries where shops deliver, with shop counts")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Countries retrieved successfully"),
            @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    public ResponseEntity<?> getCountriesWithDelivery() {
        try {
            List<com.ecommerce.dto.CountryDeliveryInfoDTO> countries = shopService.getCountriesWithDelivery();
            return ResponseEntity.ok(countries);
        } catch (Exception e) {
            log.error("Error getting countries with delivery", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                    Map.of("success", false, "message", "Failed to retrieve countries with delivery"));
        }
    }

    @GetMapping("/delivery/countries/{country}/shops")
    @Operation(summary = "Get shops delivering to a country", description = "Get shops that deliver to a specific country with pagination and search")
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> getShopsDeliveringToCountry(
        ...
        try {
            if (country == null || country.trim().isEmpty()) {
                return ResponseEntity.badRequest().body(
                        Map.of("success", false, "message", "Country parameter is required"));
            }

            Pageable pageable = PageRequest.of(page, size);
            Page<com.ecommerce.dto.ShopDeliveryInfoDTO> shopsPage = shopService.getShopsDeliveringToCountry(country.trim(), pageable);
            
            // Apply search filter if provided
            if (search != null && !search.trim().isEmpty()) {
                List<com.ecommerce.dto.ShopDeliveryInfoDTO> filteredShops = shopsPage.getContent().stream()
                        .filter(shop -> shop.getShopName().toLowerCase().contains(search.toLowerCase().trim())
                                || (shop.getDescription() != null && shop.getDescription().toLowerCase().contains(search.toLowerCase().trim())))
                        .collect(java.util.stream.Collectors.toList());
                
                // Recreate page with filtered results
                int start = (int) pageable.getOffset();
                int end = Math.min((start + pageable.getPageSize()), filteredShops.size());
                List<com.ecommerce.dto.ShopDeliveryInfoDTO> paginatedShops = filteredShops.subList(start, end);
                shopsPage = new PageImpl<>(paginatedShops, pageable, filteredShops.size());
            }
            
            return ResponseEntity.ok(shopsPage);
        } catch (Exception e) {
            log.error("Error getting shops delivering to country: {}", country, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                    Map.of("success", false, "message", "Failed to retrieve shops"));
        }
    }
