...
public class ReturnDeliveryAssignmentController {
    ...
    public ResponseEntity<?> assignDeliveryAgent(
        ...
        try {
            ...
            // Validate shop access for vendors
            if (authentication != null && authentication.getAuthorities().stream()
                    .anyMatch(a -> a.getAuthority().equals("ROLE_VENDOR"))) {
                UUID shopId = returnService.getShopIdForReturnRequest(request.getReturnRequestId());
                shopAuthorizationService.assertCanManageShop(assignedBy, shopId);
            }
            ...
            log.error("Invalid argument for delivery agent assignment: {}", e.getMessage());
            ...
            log.error("Invalid state for delivery agent assignment: {}", e.getMessage());
            Map<String, Object> errorResponse = new HashMap<>();
            errorResponse.put("success", false);
            errorResponse.put("message", e.getMessage());
            errorResponse.put("error", e.getMessage());
            ...
            Map<String, Object> errorResponse = new HashMap<>();
            String errorMessage = e.getMessage() != null ? e.getMessage() : "Failed to assign delivery agent";
            errorResponse.put("success", false);
            ...
            log.error("Unexpected error assigning delivery agent: {}", e.getMessage(), e);
            Map<String, Object> errorResponse = new HashMap<>();
            errorResponse.put("success", false);
            errorResponse.put("message", "An unexpected error occurred while assigning delivery agent");
            errorResponse.put("error", e.getMessage());
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'EMPLOYEE', 'VENDOR')")
    public ResponseEntity<?> getDeliveryAgentWorkload(
            @Parameter(description = "Delivery agent ID") @PathVariable UUID deliveryAgentId) {
        try {
            log.info("Getting workload for delivery agent: {}", deliveryAgentId);
            
            DeliveryAgentWorkloadDTO workload = deliveryAssignmentService.getDeliveryAgentWorkload(deliveryAgentId);
            
            return ResponseEntity.ok(workload);
        } catch (IllegalArgumentException e) {
            log.error("Invalid argument for delivery agent workload: {}", e.getMessage());
            Map<String, Object> errorResponse = new HashMap<>();
            errorResponse.put("success", false);
            errorResponse.put("message", e.getMessage());
            errorResponse.put("error", e.getMessage());
            ...
            log.error("Error getting delivery agent workload: {}", e.getMessage(), e);
            Map<String, Object> errorResponse = new HashMap<>();
            errorResponse.put("success", false);
            errorResponse.put("message", e.getMessage() != null ? e.getMessage() : "Failed to get delivery agent workload");
            errorResponse.put("error", e.getMessage() != null ? e.getMessage() : "Failed to get delivery agent workload");
            errorResponse.put("status", 404);
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(errorResponse);
        } catch (Exception e) {
            log.error("Unexpected error getting delivery agent workload: {}", e.getMessage(), e);
            Map<String, Object> errorResponse = new HashMap<>();
            errorResponse.put("success", false);
            errorResponse.put("message", "An unexpected error occurred while getting delivery agent workload");
            errorResponse.put("error", e.getMessage());
            errorResponse.put("status", 500);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(errorResponse);
        }
