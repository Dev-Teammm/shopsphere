...
public class OrderDeliveryNoteController {
    ...
    @Operation(summary = "Get notes for a shop order", description = "Get all delivery notes for a specific shop order")
    public ResponseEntity<Map<String, Object>> getNotesForOrder(
            ...
            Authentication authentication) {
        try {
            log.info("Fetching notes for shop order: {}", orderId);
            
            Sort.Direction direction = sortDirection.equalsIgnoreCase("ASC") ? Sort.Direction.ASC : Sort.Direction.DESC;
            Pageable pageable = PageRequest.of(page, size, Sort.by(direction, sortBy));
            
            // Pass agent email for validation (null for ADMIN/EMPLOYEE)
            String agentEmail = authentication != null && authentication.getAuthorities().stream()
                    .anyMatch(a -> a.getAuthority().equals("ROLE_DELIVERY_AGENT")) 
                    ? authentication.getName() 
                    : null;
            
            Page<OrderDeliveryNoteDTO> notes = noteService.getNotesForOrder(orderId, pageable, agentEmail);
    ...
    public ResponseEntity<Map<String, Object>> getAllNotesForDeliveryGroup(
        ...
        try {
            ...
            Page<OrderDeliveryNoteDTO> notes = noteService.getAllNotesForDeliveryGroup(groupId, pageable, agentEmail);
