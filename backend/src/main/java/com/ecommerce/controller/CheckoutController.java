...
public class CheckoutController {
    ...
    public ResponseEntity<?> createCheckoutSession(@Valid @RequestBody CheckoutRequest request) {
        ...
        } catch (com.ecommerce.Exception.CheckoutValidationException e) {
            log.warn("Checkout validation error: {}", e.getMessage());
            Map<String, Object> response = new HashMap<>();
            response.put("success", false);
            response.put("errorCode", e.getErrorCode());
            response.put("message", e.getUserMessage());
            response.put("details", e.getUserMessage());

            // Include additional data like product stock information
            if (!e.getAdditionalData().isEmpty()) {
                response.putAll(e.getAdditionalData());
            }
    ...
    public ResponseEntity<?> createGuestCheckout(@Valid @RequestBody GuestCheckoutRequest request) {
        try {
            ...
            log.warn("Checkout validation error during guest checkout: {}", e.getMessage());
            Map<String, Object> response = new HashMap<>();
            response.put("success", false);
            response.put("errorCode", e.getErrorCode());
            response.put("message", e.getUserMessage());
    ...
    public ResponseEntity<?> getPaymentSummary(
        ...
        try {
            ...
            com.ecommerce.dto.PaymentSummaryDTO summary = checkoutService.calculatePaymentSummary(
                    ...
                    request.getItems(),
                    userId,
                    request.getShopFulfillmentPreferences());
