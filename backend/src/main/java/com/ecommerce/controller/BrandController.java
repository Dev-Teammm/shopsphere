...
public class BrandController {
    ...
    public ResponseEntity<BrandDTO> createBrand(@Valid @RequestBody CreateBrandDTO createBrandDTO) {
        ...
        if (userRole == UserRole.VENDOR || userRole == UserRole.EMPLOYEE) {
            UUID shopId = createBrandDTO.getShopId();

            // If shopId is missing in the DTO, try to infer it from the user's associated
            // shops
            if (shopId == null) {
                log.info("shopId is missing for VENDOR/EMPLOYEE, attempting to infer from user shops - userId: {}",
                        currentUserId);
                List<ShopDTO> userShops = shopService.getUserShops(currentUserId);
                if (userShops.size() == 1) {
                    shopId = userShops.get(0).getShopId();
                    createBrandDTO.setShopId(shopId);
                    log.info("Inferred shopId: {} for user: {}", shopId, currentUserId);
                } else if (userShops.isEmpty()) {
                    log.error("User {} has no associated shops", currentUserId);
                    throw new IllegalArgumentException("User has no associated shops to create brands for");
                } else {
                    log.error("User {} has multiple shops and no shopId was provided", currentUserId);
                    throw new IllegalArgumentException("shopId is required for users with multiple shops");
                }
            }

            log.info("Validating shop access - userId: {}, shopId: {}", currentUserId, shopId);
