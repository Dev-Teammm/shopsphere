...
public class CategoryController {
    ...
    public ResponseEntity<CategoryDTO> createCategory(@Valid @RequestBody CategoryDTO categoryDTO) {
        ...
        if (userRole == UserRole.VENDOR || userRole == UserRole.EMPLOYEE) {
            UUID shopId = categoryDTO.getShopId();
            ...
            if (shopId == null) {
                log.info("shopId is missing for VENDOR/EMPLOYEE, attempting to infer from user shops - userId: {}",
                        currentUserId);
                List<com.ecommerce.dto.ShopDTO> userShops = shopService.getUserShops(currentUserId);
                if (userShops.size() == 1) {
                    shopId = userShops.get(0).getShopId();
                    categoryDTO.setShopId(shopId);
                    log.info("Inferred shopId: {} w: {}", shopId, currentUserId);
                } else if (userShops.isEmpty()) {
                    log.error("User {} has no associated shops", currentUserId);
                    throw new IllegalArgumentException("User has no associated shops to create categories for");
                } else {
