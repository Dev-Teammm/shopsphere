...
public class ProductWarehouseController {
    ...
    public ResponseEntity<WarehouseStockPageResponse> getProductWarehouseStock(
        ...
        try {
            ...
            // Normalize sort field for nested properties (client may send "warehouseName")
            if ("warehouseName".equals(sort)) {
                sort = "warehouse.name"; // map to JPA path
            } else if ("warehouseId".equals(sort)) {
                sort = "warehouse.id";
            }
