...
public class DeliveryAssignmentServiceImpl implements DeliveryAssignmentService {
    ...
    public DeliveryAssignmentDTO assignAgentToOrder(Long orderId, java.util.UUID agentId) {
        ...
        // Validate that at least one shop in the order supports delivery (check transition state)
        boolean hasDeliveryCapableShop = order.getShopOrders().stream()
                .anyMatch(shopOrder -> {
                    if (shopOrder.getShop() == null) return false;
                    com.ecommerce.service.CapabilityTransitionService.EffectiveCapabilities effectiveCapabilities = 
                        capabilityTransitionService.getEffectiveCapabilities(shopOrder.getShop().getShopId());
                    return effectiveCapabilities.supportsDelivery();
                });
        
        if (!hasDeliveryCapableShop) {
            throw new IllegalArgumentException("Cannot assign delivery agent: No shops in this order support delivery. This order is for pickup only.");
        }
